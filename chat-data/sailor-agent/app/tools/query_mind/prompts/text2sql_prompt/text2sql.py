...
#     3、用**dict**形式罗列数据表字段和其对应的筛选方式和筛选条件，格式如下：
#        {"字段名1": "筛选方式1筛选值1", "字段名2": "筛选方式2筛选值2", ..., "字段名n": "筛选方式n筛选值n"}；
...
- 注意 DDL 中的表名，由三部分组成，例如: vdm_xxx.default.xxx, 其中 vdm_xxx 是数据源的名称，\
default 是数据库的名称，xxx 是表的名称，有可能某些部分存在 "" 等特殊字符，请务必保留原格式，否则会出错
"""
