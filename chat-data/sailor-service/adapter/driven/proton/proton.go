...
func NewService(httpClient *http.Client) Service {
	cfg := settings.GetConfig().ProtonConf
	cli := &http.Client{
		Transport: httpClient.Transport,
		Timeout:   5 * time.Minute,
	}
	return &service{
		baseUrl:    cfg.ApplicationUrl,
		httpClient: cli,
	}
}

type LicenseResp struct {
	AnyFabric []string `json:"anyfabric"`
	AnyRobot  []string `json:"anyrobot"`
	AnyShare  []string `json:"anyshare"`
}

func (s *service) GetLicense(ctx context.Context, req map[string]any) (*LicenseResp, error) {
	var err error
	ctx, span := trace.StartInternalSpan(ctx)
	defer func() { trace.TelemetrySpanEnd(span, err) }()

	rawURL := s.baseUrl + "/api/proton-application/v1/license"
	...
	return httpGetDo[LicenseResp](ctx, u, s)
}

func httpGetDo[T any](ctx context.Context, u *url.URL, a *service) (*T, error) {
...
func httpDo[T any](ctx context.Context, httpMethod, url string, body io.Reader, bodyStr string, headers url.Values, a *service) (*T, error) {
	var err error
	ctx, span := trace.StartInternalSpan(ctx)
	defer func() { trace.TelemetrySpanEnd(span, err) }()
